from __future__ import annotations

import asyncio
import time
from datetime import UTC, datetime
from typing import cast

from my_agent.core.bus.events import (
    PermissionDeniedEvent,
    PermissionGrantedEvent,
    PermissionRequestedEvent,
    ToolCallFailedEvent,
    ToolCallFinishedEvent,
    ToolCallStartedEvent,
)
from my_agent.core.command.evidence import evidence_for_tool_call
from my_agent.core.command.recovery import classify_command_failure
from my_agent.core.events.bus import EventBus
from my_agent.core.llm.types import ToolCallBlock
from my_agent.core.permissions.manager import PermissionManager
from my_agent.core.tools.base import ToolResult
from my_agent.core.tools.registry import ToolRegistry

_DEFAULT_TIMEOUT = 10.0


def _now() -> str:
    return datetime.now(UTC).isoformat()


async def _fail(
    bus: EventBus,
    run_id: str,
    tool_call: ToolCallBlock,
    error_type: str,
    error_message: str,
    elapsed_ms: int,
    *,
    workspace_root: str | None = None,
    step: int | None = None,
) -> ToolResult:
    evidence = evidence_for_tool_call(
        tool_call.name,
        dict(tool_call.input),
        run_id=run_id,
        step=step,
        tool_use_id=tool_call.id,
        workspace_root=workspace_root,
    )
    if evidence is not None:
        evidence.error_type = classify_command_failure(error_type, error_message)

    await bus.publish(
        ToolCallFailedEvent(
            run_id=run_id,
            tool_use_id=tool_call.id,
            tool_name=tool_call.name,
            error_type=error_type,
            error_message=error_message,
            elapsed_ms=elapsed_ms,
            evidence=evidence,
            ts=_now(),
        )
    )
    return ToolResult(content=error_message, is_error=True, error_type=error_type)


async def invoke_tool(
    registry: ToolRegistry,
    tool_call: ToolCallBlock,
    bus: EventBus,
    run_id: str,
    timeout: float = _DEFAULT_TIMEOUT,
    *,
    permission_manager: PermissionManager | None = None,
    session_id: str = "",
    workspace_root: str | None = None,
    step: int | None = None,
) -> ToolResult:
    t0 = time.monotonic()
    evidence = evidence_for_tool_call(
        tool_call.name,
        dict(tool_call.input),
        run_id=run_id,
        step=step,
        tool_use_id=tool_call.id,
        workspace_root=workspace_root,
    )

    await bus.publish(
        ToolCallStartedEvent(
            run_id=run_id,
            tool_use_id=tool_call.id,
            tool_name=tool_call.name,
            params=dict(tool_call.input),
            evidence=evidence,
            ts=_now(),
        )
    )

    def elapsed() -> int:
        return int((time.monotonic() - t0) * 1000)

    tool = registry.get(tool_call.name)
    if tool is None:
        return await _fail(
            bus,
            run_id,
            tool_call,
            "runtime_error",
            f"unknown tool: {tool_call.name}",
            elapsed(),
            workspace_root=workspace_root,
            step=step,
        )

    required: list[str] = cast(list[str], tool.input_schema.get("required", []))
    missing = [p for p in required if p not in tool_call.input]
    if missing:
        return await _fail(
            bus,
            run_id,
            tool_call,
            "schema_error",
            f"missing required parameters: {', '.join(missing)}",
            elapsed(),
            workspace_root=workspace_root,
            step=step,
        )

    if permission_manager is not None:
        async def emit_permission(event: dict[str, object]) -> None:
            await bus.publish(
                PermissionRequestedEvent(
                    run_id=run_id,
                    tool_use_id=str(event["tool_use_id"]),
                    tool_name=str(event["tool_name"]),
                    params=dict(tool_call.input),
                    session_id=session_id,
                    ts=_now(),
                )
            )

        allowed, decision = await permission_manager.check_and_wait(
            tool_use_id=tool_call.id,
            tool_name=tool_call.name,
            params=dict(tool_call.input),
            session_id=session_id,
            event_emitter=emit_permission,
        )
        if allowed:
            if decision != "auto_allow":
                await bus.publish(
                    PermissionGrantedEvent(
                        run_id=run_id,
                        tool_use_id=tool_call.id,
                        decision=decision,
                        ts=_now(),
                    )
                )
        else:
            if decision != "auto_deny":
                await bus.publish(
                    PermissionDeniedEvent(
                        run_id=run_id,
                        tool_use_id=tool_call.id,
                        decision=decision,
                        ts=_now(),
                    )
                )
            return await _fail(
                bus,
                run_id,
                tool_call,
                "permission_denied",
                f"permission denied: {decision}",
                elapsed(),
                workspace_root=workspace_root,
                step=step,
            )

    try:
        result = await asyncio.wait_for(
            tool.invoke(dict(tool_call.input)),
            timeout=timeout,
        )
        ms = elapsed()
        if result.is_error:
            return await _fail(
                bus,
                run_id,
                tool_call,
                result.error_type or "runtime_error",
                result.content,
                ms,
                workspace_root=workspace_root,
                step=step,
            )
        await bus.publish(
            ToolCallFinishedEvent(
                run_id=run_id,
                tool_use_id=tool_call.id,
                tool_name=tool_call.name,
                elapsed_ms=ms,
                evidence=evidence,
                ts=_now(),
            )
        )
        return result
    except TimeoutError:
        return await _fail(
            bus,
            run_id,
            tool_call,
            "timeout",
            f"tool timed out after {timeout}s",
            elapsed(),
            workspace_root=workspace_root,
            step=step,
        )
    except Exception as exc:
        return await _fail(
            bus,
            run_id,
            tool_call,
            "runtime_error",
            str(exc),
            elapsed(),
            workspace_root=workspace_root,
            step=step,
        )
